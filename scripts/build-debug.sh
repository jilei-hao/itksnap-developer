#!/usr/bin/env bash
# build-debug.sh — Configure and build ITK-SNAP in Debug mode.
# Run from itksnap-developer/ root, or pass DEVDIR as env var.
#
# Usage:
#   scripts/build-debug.sh [OPTIONS]
#
# Options:
...
#   -b, --build <path>  Build directory (default: <devdir>/build-debug)
...
BUILD_DIR="${BUILD_DIR:-$DEVDIR/build-debug}"
