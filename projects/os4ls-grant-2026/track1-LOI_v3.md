...
> sponsor).
> **Update:** added hard adoption metrics (11,082 citations; 1.16M+ SourceForge downloads)
> and reduced repetition of "human-in-the-loop" (now anchored once, as the Aim 1 name).
...
biomedical images, with a ~20-year track record, 11,082 citations and over 1.16M downloads,
and a large international user base across neuroimaging, radiology, and cardiology. It
already integrates AI: our shipped itksnap-dls server brings foundation-model segmentation
...
cycles. Critically, the tools that do involve a human keep that step inside their own labeling loop — there is no way for an outside pipeline to call on expert judgment as a reusable step and get the corrected result back.
...
capability is a set of human-review primitives: review/correction as a callable, resumable
step (request_review), and capture of expert interactions (clicks, scribbles, edits,
decisions) as machine-consumable labels and provenance — making expert judgment a
...
losslessly to and from other tools (e.g. 3D Slicer), making ITK-SNAP the human checkpoint
...
interactions captured as machine-consumable labels and provenance; a data engine that turns
corrections into training data; and reproducible, scriptable segmentation / registration /
4D workflows callable from Python and agents.
...
AI enablement / large-scale data: making expert judgment orchestrable enables active
learning and large-cohort, auditable training-data generation — the data-prep that
underpins model training and evaluation — with remote/partial data access for scale.

*(1,451 chars — verified.)*
...
suites (Materialise Mimics, Synopsys Simpleware) and vendor workstations (syngo.via, MIM).
In adjacent microscopy, napari is widely used.

On AI specifically, the closest comparator is MONAI Label: an AI-assisted interactive
...
and ITK-SNAP — so model-assisted interactive segmentation is becoming common across these
tools.

ITK-SNAP is among the most established and most cited tools in the space: ~20 years of
development, 11,082 citations of its methods paper, 1.16M+ SourceForge downloads, a large
international user base, and a reputation as the most approachable tool for fast 3D
segmentation. It is fully open source, cross-platform, and actively maintained.
...
driven by an external agent, not a labeling loop, composable with registration, 4D
propagation, and open formats. This proposal closes that gap, bringing a trusted tool fully
into AI-native research.

*(1,497 chars — verified; very tight, re-check the portal cap.)*
...
## Adoption metrics (for Existing Impact / Landscape)

- **Citations:** 11,082 (ITK-SNAP methods paper).
- **Downloads:** 1,163,932 total on SourceForge.
- *(When citing in the form, note the source/as-of date for each figure so reviewers can
  verify; the guide says metrics may be cross-checked against public sources.)*
...
- Confirm **DICOM-SEG round-trip scope** with a 3D Slicer user (gates Aim 2.5
  interoperability).
- Re-verify all four section character counts **in the portal** (Landscape sits only ~3
  chars under the cap by my count).
